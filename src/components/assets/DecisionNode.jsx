...
class DecisionNodeComponenent extends React.Component {
  ...
  enter = () => {
    ...
    this.d3Node.datum(this.props.node).attr('transform', node => {
      if (typeof node.parent == 'undefined') {
        console.log('no parent enter', node)
        return `translate(${this.props.height / 2}, ${ 0})`;
      } else
        return `translate(${node.source.y0}, ${node.source.x0})`;
      }
    ).transition().duration(this.props.duration).attr('transform', node => {
      return `translate(${node.y}, ${node.x})`;
    })
    this.state.node.x0 = this.state.node.x;
    this.state.node.y0 = this.state.node.y;
  ...
  render() {
    ...
    const hasChildren = this.props.node.children || this.props.node._children
      ? true
      : false;
